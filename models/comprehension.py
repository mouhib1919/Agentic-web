...
easily an AI agent can understand the semantic structure, meaning, and
organization of a website. No evidence collection (HTTP requests, HTML
parsing, structured-data extraction, crawling) belongs here — this is
a data container only.
"""
...
class ComprehensionResult:
    ...
    Attributes:
        ...
        checks: Pass/fail outcome of each individual criterion, keyed
            by criterion name (e.g. `"structured_data"`, `"json_ld"`).
        details: Supporting evidence for each criterion (raw values
            such as discovered Schema.org types, title, or language)
