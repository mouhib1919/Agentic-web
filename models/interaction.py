...
Interaction Agent: a scored, JSON-serializable judgment of how easily
an AI agent can interact with a website through available
machine-readable and actionable interfaces. No evidence collection
(HTTP requests, HTML parsing, API discovery, frontend static analysis)
...
class InteractionResult:
    """Scored assessment of a website's interactivity for AI agents.
    ...
    Attributes:
        ...
        checks: Pass/fail outcome of each individual criterion, keyed
            by criterion name (e.g. `"api_availability"`, `"graphql"`).
        details: Supporting evidence for each criterion (raw values
            such as the discovered API endpoints, GraphQL endpoints,
            or MCP tools) useful for reporting and debugging.
